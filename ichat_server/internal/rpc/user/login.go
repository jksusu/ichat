...
func (*user) Login(username string, password string) (*pb.LoginResponse, error) {
	list := ichat_model.UserModel.ListByUsername(username)
	if list == nil {
	...
	if data.Token != "" {
		return &pb.LoginResponse{Uid: data.Uid, Token: data.Token}, nil
	}
	//生成token
	token := rand.GenToken()
	...
	if ok := user_cache.UserCache.TokenCache(token, userInfo, time.Hour*24); !ok {
		return &pb.LoginResponse{Uid: data.Uid, Token: token}, errors.New("登录失败，请重试")
	}
	list.Token = token
	db.DB.Save(&list)

	return &pb.LoginResponse{Uid: data.Uid, Token: token}, nil
}
